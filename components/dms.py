...
def dms_callback(mqtt_client, settings, value, alarm: Alarm, security_system: SecuritySystem):
    mqtt_client.publish("commands/dms", json.dumps({"password": value}))
...
def run_dms(mqtt_client, settings, batch, data_lock, threads, stop_event):
        if settings['simulated']:
            ...
            dms = DMS(settings, batch, data_lock, mqtt_client)
